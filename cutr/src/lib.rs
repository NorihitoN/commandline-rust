    ...
    let matches = App::new("cutr")
        ...
        .arg(
            Arg::with_name("delimiter")
                .value_name("DELIM")
                .help("use DELIN instead of TAB for field delimiter")
                .short("d")
                .long("delim")
                .default_value("\t"),
        )
        .arg(
            Arg::with_name("bytes")
                ...
                .long("bytes")
                .takes_value(true)
                .conflicts_with_all(&["delimiter"]),
        )
        .arg(
            Arg::with_name("chars")
                .value_name("CHARS")
                .help("select only these chars")
                .short("c")
                .long("charactors")
                .takes_value(true)
                .conflicts_with_all(&["bytes", "delimiter"]),
        )
        .arg(
...
fn extract_fields(record: &StringRecord, field_pos: &[Range<usize>]) -> Vec<String> {
    field_pos
        .iter()
        .cloned()
        .flat_map(|range| range.filter_map(|pos| record.get(pos)))
        .map(String::from)
        .collect()
}
...
fn parse_index(input: &str) -> Result<usize, String> {
    let val_err = || format!("illegal list value: \"{}\"", input);
    input
        .starts_with('+')
        .then(|| Err(val_err()))
        .unwrap_or_else(|| {
